...
import androidx.compose.compiler.plugins.kotlin.lower.hiddenfromc.hiddenFromCClassId
...
//        context.irBuiltIns.greaterFunByOperandType
    ...
    private val hiddenFromCIrClass: IrClass? =
        if (context.platform.isNative()) {
            getTopLevelClass(hiddenFromCClassId).owner
        } else {
            null
        }
    ...
    private fun hiddenFromC() = hiddenFromCIrClass?.let {
        IrConstructorCallImpl.fromSymbolOwner(
            type = it.defaultType,
            constructorSymbol = it.constructors.first().symbol
        )
