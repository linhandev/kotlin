 ...
 * NUMBER: 1
 * DESCRIPTION: in operator on lazy delegated Box receiver resolves extension contains at runtime
 */
...
fun test(w: Wrapper): Boolean = 2 in w.inner

fun box(): String {
    val w = Wrapper(Box(listOf(1, 2, 3)))
    if (!test(w)) return "NOK: present element via lazy delegate"
    if (test(Wrapper(Box(listOf(4, 5))))) return "NOK: absent element via lazy delegate"
    return "OK"
}
