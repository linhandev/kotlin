...
template <typename T>
HAS_SAFEPOINT_THROW inline void fillImpl(KRef thiz, KInt fromIndex, KInt toIndex, T value)
{
...
template <typename T>
HAS_SAFEPOINT_THROW inline void copyImpl(KConstRef thiz, KInt fromIndex, KRef destination, KInt toIndex, KInt count)
{
    ...
    if (count < 0 || fromIndex < 0 || static_cast<uint32_t>(count) + fromIndex > array->count_ || toIndex < 0 ||
        static_cast<uint32_t>(count) + toIndex > destinationArray->count_) {
        ThrowArrayIndexOutOfBoundsException();
    }
    mutabilityCheck(destination);
    memmove(PrimitiveArrayAddressOfElementAt<T>(destinationArray, toIndex), PrimitiveArrayAddressOfElementAt<T>(array, fromIndex),
            count * sizeof(T));
}
...
HAS_SAFEPOINT_THROW
void Kotlin_Array_copyImpl(KConstRef thiz, KInt fromIndex, KRef destination, KInt toIndex, KInt count)
{
    ...
    if (fromIndex >= toIndex) {
        for (int index = 0; index < count; index++) {
            UpdateHeapRef(
                    ArrayAddressOfElementAt(destinationArray, toIndex + index),
                    ReadHeapRef(ArrayAddressOfElementAt(array, fromIndex + index), array->obj()), destinationArray->obj());
        }
    } else {
...
#if KONAN_NO_UNALIGNED_ACCESS
    const uint8_t* address = reinterpret_cast<const uint8_t*>(ByteArrayAddressOfElementAt(array, index));
    return (static_cast<KInt>(address[0]) << 0) | (static_cast<KInt>(address[1]) << 8) | (static_cast<KInt>(address[2]) << 16) |
            (static_cast<KInt>(address[3]) << 24);
#else
...
#if KONAN_NO_UNALIGNED_ACCESS
    const uint8_t* address = reinterpret_cast<const uint8_t*>(ByteArrayAddressOfElementAt(array, index));
    return (static_cast<KLong>(address[0]) << 0) | (static_cast<KLong>(address[1]) << 8) | (static_cast<KLong>(address[2]) << 16) |
            (static_cast<KLong>(address[3]) << 24) | (static_cast<KLong>(address[4]) << 32) | (static_cast<KLong>(address[5]) << 40) |
...
OBJ_GETTER(Kotlin_CharArray_copyOf, KConstRef thiz, KInt newSize) {
    ...
    memcpy(PrimitiveArrayAddressOfElementAt<KChar>(dst, 0), PrimitiveArrayAddressOfElementAt<KChar>(holder.obj()->array(), 0),
           toCopy * sizeof(KChar));
    RETURN_OBJ(dst->obj());
}
...
HAS_SAFEPOINT_THROW
void Kotlin_ByteArray_copyImpl(KConstRef thiz, KInt fromIndex, KRef destination, KInt toIndex, KInt count)
{
    copyImpl<KByte>(thiz, fromIndex, destination, toIndex, count);
}

HAS_SAFEPOINT_THROW
void Kotlin_ShortArray_copyImpl(KConstRef thiz, KInt fromIndex, KRef destination, KInt toIndex, KInt count)
{
    copyImpl<KShort>(thiz, fromIndex, destination, toIndex, count);
}

HAS_SAFEPOINT_THROW
void Kotlin_CharArray_copyImpl(KConstRef thiz, KInt fromIndex, KRef destination, KInt toIndex, KInt count)
{
    copyImpl<KChar>(thiz, fromIndex, destination, toIndex, count);
}

HAS_SAFEPOINT_THROW
void Kotlin_IntArray_copyImpl(KConstRef thiz, KInt fromIndex, KRef destination, KInt toIndex, KInt count)
{
    copyImpl<KInt>(thiz, fromIndex, destination, toIndex, count);
}

HAS_SAFEPOINT_THROW
void Kotlin_LongArray_copyImpl(KConstRef thiz, KInt fromIndex, KRef destination, KInt toIndex, KInt count)
{
    copyImpl<KLong>(thiz, fromIndex, destination, toIndex, count);
}

HAS_SAFEPOINT_THROW
void Kotlin_FloatArray_copyImpl(KConstRef thiz, KInt fromIndex, KRef destination, KInt toIndex, KInt count)
{
    copyImpl<KFloat>(thiz, fromIndex, destination, toIndex, count);
}

HAS_SAFEPOINT_THROW
void Kotlin_DoubleArray_copyImpl(KConstRef thiz, KInt fromIndex, KRef destination, KInt toIndex, KInt count)
{
    copyImpl<KDouble>(thiz, fromIndex, destination, toIndex, count);
}

HAS_SAFEPOINT_THROW
void Kotlin_BooleanArray_copyImpl(KConstRef thiz, KInt fromIndex, KRef destination, KInt toIndex, KInt count)
{
...
OBJ_GETTER(Kotlin_ImmutableBlob_toByteArray, KConstRef thiz, KInt startIndex, KInt endIndex) {
    ...
    memcpy(PrimitiveArrayAddressOfElementAt<KByte>(dst, 0), PrimitiveArrayAddressOfElementAt<KByte>(holder.obj()->array(), startIndex),
           len);
    RETURN_OBJ(dst->obj());
}
