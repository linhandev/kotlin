...
_Kotlin_N2KStub:
        ...
        stp  q4, q5, [sp, #ForwardStubCalleeSaveArea+0x90]
        cfi_rel_offset (q4, ForwardStubCalleeSaveArea+0x90)
        cfi_rel_offset (q5, ForwardStubCalleeSaveArea+0xa0)

        stp  q6, q7, [sp, #ForwardStubCalleeSaveArea+0xb0]
        cfi_rel_offset (q6, ForwardStubCalleeSaveArea+0xb0)
        cfi_rel_offset (q7, ForwardStubCalleeSaveArea+0xc0)
        ...
        stp  x9, x10, [sp, #ForwardStubCalleeSaveArea+0xd0]
        cfi_rel_offset (x9, ForwardStubCalleeSaveArea+0xd0)
        cfi_rel_offset (x10, ForwardStubCalleeSaveArea+0xd8)
        ...
        ldp  q4, q5, [sp, #ForwardStubCalleeSaveArea+0x90]
        cfi_restore (q4)
        cfi_restore (q5)

        ldp  q6, q7, [sp, #ForwardStubCalleeSaveArea+0xb0]
        cfi_restore (q6)
        cfi_restore (q7)

        ldp  x9, x10, [sp, #ForwardStubCalleeSaveArea+0xd0]
        cfi_restore (x9)
        cfi_restore (x10)
...
Lpc_n2k:
    ...
    // Export the return-address PC as a .quad so the linker does not
    // see a non-private label inside the CFI region (which would create
    // a bogus compact-unwind entry with encoding=0).
    .section __DATA,__const
    .p2align 3
    .global _unwindPCForN2KStub
_unwindPCForN2KStub:
    .quad Lpc_n2k