...
| `splitBCfile`           | Bitcode split partition count for `-Xbinary=splitBCfile=N`. Default `2`. Effective only for `ohos_arm64` with `optimizationMode=OPT` (debug / other targets force `1`). Pass `1` to disable split. |
