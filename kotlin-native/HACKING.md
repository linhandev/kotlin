...
| `sanitizer`             | Run tests with sanitizer: `NONE` (default), `THREAD`, `ADDRESS`, `HWADDRESS`. |
| `splitBCfile`           | Bitcode split partition count for `-Xbinary=splitBCfile=N`. Default `1` (no split). Effective only for `ohos_arm64` with `optimizationMode=OPT` (debug / other targets force `1`). Pass `N>1` (e.g. `-Pkn.splitBCfile=2`) to enable split. |
